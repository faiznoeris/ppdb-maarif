...
class Calonsiswa extends CI_Controller{
	...
	function delete(){
		...
		if($this->m_calonsiswa->delete($id)){
			$this->session->set_tempdata('toastDeleteCalonSiswa', true, 3);
			$this->session->set_tempdata('toastDeleteCalonSiswaID', $id , 3);
	...
	function saveedit(){
		...
		if(!empty($_POST)){
			...
			*/
			...
			if($this->m_calonsiswa->savechanges($data,$id)){
				redirect('dashboard/datapendaftar');
			}else{
				redirect('dashboard/datapendaftar');
			}
