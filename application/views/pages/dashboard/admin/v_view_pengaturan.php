 ...
 <div class="row">
  <div class="col s12">
    <ul class="tabs grey darken-1 z-depth-2">
      <li class="tab col s3"><a class="white-text waves-effect waves-light tooltipped" data-position="left" data-delay="50" data-tooltip="Set Tahun Ajaran" href="#tahun_ajaran">Tahun Ajaran</a></li>
      <li class="tab col s3"><a class="white-text waves-effect waves-light tooltipped" data-position="top" data-delay="50" data-tooltip="Set Kuota" href="#kuota">Kuota</a></li>
      <li class="tab col s3"><a class="white-text waves-effect waves-light tooltipped" data-position="right" data-delay="50" data-tooltip="Set Biaya" href="#biaya">Biaya</a></li>
      <li class="tab col s3"><a class="white-text waves-effect waves-light tooltipped" data-position="top" data-delay="50" data-tooltip="Set Bobot Nilai" href="#bobotnilai">Bobot Nilai</a></li>
...
<div class="row" id="biaya">
  <div id="admin" class="col s12">
    <div class="card material-table">
      <div class="table-header">
        <span class="table-title">Data Biaya</span>
        <div class="actions">
          <a href="<?php echo base_url('dashboard/pengaturan/biaya/add');?>" class="add_circle-toggle waves-effect btn-flat nopadding"><i class="material-icons">add_circle</i></a>
          <a href="#" class="search-toggle waves-effect btn-flat nopadding"><i class="material-icons">search</i></a>
        </div>
      </div>
      <table id="datatable3">
        ...
        <tbody>
          ...
          foreach ($data_biaya as $row) {
            ${$pre . $row->id_tahun . $row->jurusan} = 0;
          }

          foreach ($data_biaya as $row) {
            //${$pre . $row->id_tahun . $row->jurusan} = 0;

            if(isset(${$pre . $row->id_tahun . $row->jurusan})){

              ${$pre . $row->id_tahun . $row->jurusan} = ${$pre . $row->id_tahun . $row->jurusan} + $row->nominal;
          ...
          foreach ($data_biaya as $row) {
            if(!isset(${"status" . $row->id_tahun . $row->jurusan}) ){
              ${"status" . $row->id_tahun . $row->jurusan} = true;
              echo "<tr>
              <td>".$row->tahun."</td>
              <td>".$row->jurusan."</td>
              <td>Rp. ".${$pre . $row->id_tahun . $row->jurusan}."</td>
              <td>
              <a class='btn-floating btn-small tooltipped yellow darken-2' data-position='top' data-delay='50' data-tooltip='Edit Kuota' href='". base_url('dashboard/pengaturan/biaya/edit/'.$row->id_tahun.'/'.$row->jurusan) . "'>
              <i class='material-icons'>mode_edit</i>
              </a>
              </td>
              </tr>";
            }
            $thafter = $row->tahun.$row->jurusan;
          }
          ?>
